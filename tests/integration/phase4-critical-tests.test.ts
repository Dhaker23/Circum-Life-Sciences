...
describe("T-AI-GUARD-01: AI governance (PRD section 9)", () => {
  it("no quality.*.approve or quality.*.close permission is grantable to a non-human", async () => {
    ...
    approvePerms.forEach((p) => expect(p.key).toMatch(/(quality\.(deviation\.approve|change\.approve|capa\.close)|lab\.(specification\.approve|testresult\.disposition)|docs\.document\.approve|training\.competency\.authorize|equipment\.qualification\.approve)/));
  });
