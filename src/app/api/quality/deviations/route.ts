...
import { CreateDeviationSchema } from "@/modules/quality/domain";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const r = await svc.listDeviations(ctx, page, pageSize);
    return ok(r.items, { page: r.page, pageSize: r.pageSize, total: r.total });
  } catch (e) { return fail(e); }
}
export async function POST(req: NextRequest) {
  try {
    const ctx = await requirePermission("quality.deviation.create");
    const body = parseOrThrow(CreateDeviationSchema, await req.json());
    return ok(await svc.createDeviation(ctx, body));
  } catch (e) { return fail(e); }
}
