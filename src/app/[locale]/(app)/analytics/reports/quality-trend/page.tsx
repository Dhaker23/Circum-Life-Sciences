...
const METRICS = [
  { key: "fpy", color: "hsl(var(--primary))" },
  { key: "scrapRate", color: "#ef4444" },
  { key: "reworkRate", color: "#f59e0b" },
];
...
export default function QualityTrendReportPage() {
  ...
  const dataQ = useQuery({
    queryKey: ["analytics", "quality-trend", effectiveSiteId, range, granularity],
    queryFn: async () => {
      const res = await fetch("/api/analytics/reports/quality-trend", {
        method: "POST",
  ...
  const handleExport = async () => {
    ...
    a.href = url;
    a.download = `circum-quality-trend-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={t("reports.qualityTrend")} subtitle={t("reports.subtitle")} />
      ...
      {dataQ.isLoading ? <AnalyticsSkeleton cards={3} /> :
        ...
        dataQ.data ? (
          <>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-sm">{t("reports.qualityTrend")}</CardTitle>
              </CardHeader>
              <CardContent>
                {chartData.length === 0 ? (
