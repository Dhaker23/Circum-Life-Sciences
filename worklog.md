...
---
Task ID: ui3-sub2
Agent: ui3-sub2 (Z.ai Code)
Task: Implement UI-3 Sub-phase 2 for Circum — split-screen branded sign-in, dashboard visualization improvements (Quality Overview, Production Status, System Health, KPI hover tooltips + progress bars, colored activity borders), and a reusable FormField component applied to the NCR detail page transition dialogs.

Work Log:
- Read `worklog.md` (UI-1 + d005 + pag-batch1/2 + ui3-integrate context) to understand the project's existing patterns and the StatCard / StatusBadge / PageHeader / ActivityTimeline component contracts.
- Inspected the 3 target files (`sign-in/page.tsx`, `(app)/page.tsx`, `quality/ncrs/[id]/page.tsx`) + 3 reference components (`stat-card.tsx`, `status-badge.tsx`, `page-header.tsx`) + the `/api/health` route + `globals.css` (brand color = medical teal `oklch(0.45 0.12 180)`, no indigo/blue) + `tw-animate-css` package (provides `animate-in fade-in-0 slide-in-from-top-*` classes for Tailwind v4).
- Added 9 new i18n keys per locale × 3 locales (en/fr/ar): `auth.emailRequired/emailInvalid/passwordRequired/welcomeBack/securePlatform` + `common.qualityOverview/productionStatus/systemHealth/fieldRequired`. Verified all 3 JSON files parse cleanly via `JSON.parse`.

Files modified (6):
1. `src/app/[locale]/sign-in/page.tsx` — full rewrite. Split-screen on `lg+` (brand panel left `bg-primary` with Circum wordmark + tagline + decorative lucide iconography ShieldCheck/Activity/ClipboardCheck/Factory at `opacity-15`; form panel right). Mobile stacks: compact Circum logo header above the form. Inline validation: email validates on blur/submit (required + RFC-5322-subset regex), password validates on blur/submit (required). Loading state: button shows `Loader2` spinner + `signingIn` text. Success feedback: green `CheckCircle2` flash + button text before 450 ms redirect. Suspense wrapper preserved. `noValidate` on form to suppress browser-native popups. RTL: all spacing uses logical `ms-*`/`ps-*`/`start-*`/`end-*`. Email/password fields have leading Mail/Lock icons. `aria-invalid` + `aria-describedby` wired to error `<p role="alert">` with `animate-in fade-in-0` for subtle fade-in.
2. `src/app/[locale]/(app)/page.tsx` — server component preserved. Extended the existing `Promise.all` with total counts (`totalNcrs`, `totalCapas`, `totalWorkOrders`, `auditTotal`) so progress percentages are computed server-side from real Prisma counts (no fabricated data). Added `severity` to NCR `findMany` select. Added server-side System Health probe (database `SELECT 1` latency, AI provider `getProvider().health()`, active integration count) — same 3 checks as `/api/health`, wrapped in try/catch so a failing probe degrades gracefully. Added: (a) **Quality Overview** card with `Progress` bar of open/total NCRs + colored legend dots; (b) **Production Status** card with active/completed counts + `Progress` bar of `completionRate` (replaces the original `productionOverview` card); (c) **System Health** mini-card with 3 `HealthRow` items (Database, AI provider, Integrations); (d) KPI StatCards now pass `progress` (open/total ratio) + `tooltip` ("X / Y"); (e) **Recent Quality Activity** list items now have `border-s-2` colored by status/severity (red for CRITICAL severity or REJECTED/CANCELLED/FAILED, amber for INVESTIGATION/DRAFT/OPEN/PENDING, emerald for CLOSED/APPROVED/COMPLETED, slate fallback). Quick Actions card moved to bottom full-width with `sm:grid-cols-4`. All RBAC permission gates preserved.
3. `src/components/app/stat-card.tsx` — additive extension. New optional `progress?: number` prop (0–100, clamped) renders a thin 1px bar under the KPI value, color follows the card's accent (`bg-primary`/`bg-emerald-500`/`bg-amber-500`/`bg-red-500`/`bg-muted-foreground/50`). New optional `tooltip?: string` prop applied as native `title` attribute on the wrapping `Link` (interactive) or `<div>` (non-interactive). Existing props/behavior unchanged — backward compatible.
4. `src/components/app/form-field.tsx` — NEW reusable component. Props: `{ label, required?, error?, description?, htmlFor?, className?, children }`. Renders shadcn `<Label>` with label text + red `*` asterisk (when required) + `sr-only` "(required)". Children rendered as-is. When `error` is non-empty: renders `<p role="alert">` with `text-destructive text-xs` + `animate-in fade-in-0 slide-in-from-top-0.5 duration-150` for subtle fade-in. When no error but `description` provided: renders muted helper text (hidden when error is shown to avoid double-stacking). Pure presentational — caller decides when to surface errors.
5. `src/app/[locale]/(app)/quality/ncrs/[id]/page.tsx` — `TransitionDialog` updated. Added `submitAttempted` state + `setSubmitAttempted(true)` at the top of `handleSubmit()` (before the `canSubmit()` early-return). Field-level error computation: `reasonError`/`containmentError`/`dispositionError`/`closureError` — each is `fieldRequiredLabel` when `submitAttempted && <field empty>`, else `null`. All 4 form fields (reason, containmentAction, disposition select, closureNotes) now wrapped in `<FormField label=... required error=... htmlFor=...>`. Each control has `aria-invalid={Boolean(<field>Error)}`. `TransitionDialogProps` interface + caller site updated to thread `fieldRequiredLabel={tc("fieldRequired")}` through. Removed unused `Label` import. `reset()` now also clears `submitAttempted`. Success/error toasts preserved verbatim. Other 4 detail pages (Deviation, CAPA, ChangeControl, BatchReview) untouched per spec.
6. `src/messages/{en,fr,ar}.json` — 9 new keys per locale (auth + common namespaces).

Decisions / pattern notes:
- **Sign-in success flash (450 ms)**: long enough for the user to register the green `CheckCircle2` + button text change, short enough not to feel slow. Then `router.push(callbackUrl)`.
- **Sign-in `noValidate`**: suppresses browser-native popups so our i18n-localized inline errors are the single source of truth.
- **Dashboard System Health probe runs server-side**: same 3 checks as `/api/health` (db `$queryRaw\`SELECT 1\``, `getProvider().health()`, `integrationConfig.count`). Avoids an HTTP roundtrip to its own route (unreliable server-side in Next 16). Wrapped in try/catch so a failing probe degrades to "degraded"/"unhealthy" instead of crashing the dashboard.
- **Dashboard: no fabricated data**: every number comes from a real Prisma count. `closedNcrs = totalNcrs - openNcrs` is derived. Progress percentages computed server-side, clamped to [0,100] in StatCard.
- **Dashboard activity border color priority**: NCR severity (CRITICAL/MAJOR/MINOR) wins over workflow status, because severity is the more meaningful triage signal. For Deviations/CAPAs (no severity in fetch), workflow status is used. Color map mirrors StatusBadge's success/warning/error semantics.
- **StatCard progress + tooltip are additive**: existing callers (other dashboards, list pages) don't pass these props and see no visual change. Only the dashboard opts in.
- **FormField: error vs. description**: only one shown at a time. When error is present, description is hidden (avoids conflicting guidance). Standard shadcn form pattern.
- **NCR detail: validation timing**: errors surface only after `submitAttempted` is true (avoids yelling on first focus). Once surfaced, they re-evaluate on every keystroke via controlled-component re-renders.
- **NCR detail: `submitAttempted` reset on dialog close**: via the existing `reset()` function. Re-opening the dialog doesn't show stale errors.
- **Other 4 detail pages untouched**: per spec, only NCR is the proof-of-pattern. Deviation/CAPA/ChangeControl/BatchReview can adopt `<FormField>` in a future sub-phase.

Verification:
- `bunx tsc --noEmit 2>&1 | grep -v vitest | head -10` → **0 errors, 0 output** (clean typecheck, exit 0).
- `bun run lint` → **0 errors, 111 warnings** (all pre-existing `no-explicit-any`/`no-console` in modules/services/tests — same baseline as previous tasks). Verified via grep: ZERO warnings reference any of the 6 modified file paths.
- i18n JSON validity: `JSON.parse` on all 3 locale files → all parse cleanly. New keys verified present in all 3 locales.
- Dev server was idle during this task (no requests in `dev.log` beyond the existing `/api/health` probe); changes will be picked up on next request. Typecheck + lint pass confirm structural correctness.

Stage Summary:
- **Task 1 (Sign-in branding)**: split-screen enterprise look with brand panel (Circum logo, tagline, decorative medical/industrial iconography) + form panel. Inline email/password validation on blur/submit. Loading spinner + success flash before redirect. Suspense wrapper preserved. All existing i18n keys reused; 5 new keys added.
- **Task 2 (Dashboard viz)**: 4 new visual elements — Quality Overview card (Progress bar of open/total NCRs), Production Status card (active/completed counts + Progress bar), System Health mini-card (database/AI/integrations status), KPI StatCard progress bars + hover tooltips. Activity list now has colored left borders by status/severity. Server component architecture preserved; no fabricated data.
- **Task 3 (Form UX)**: reusable `<FormField>` component created. NCR detail page's 4 transition-dialog fields (reason, containmentAction, disposition, closureNotes) wrapped in `<FormField>` with required asterisks + field-level errors on submit attempt. Submit button already disabled when invalid. Success/error toasts preserved. Other 4 detail pages untouched (future sub-phase).
